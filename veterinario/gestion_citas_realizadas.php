...
try {
    $sql_count = $conexion->prepare("SELECT COUNT(c.id) FROM citas c WHERE c.estado = 'realizada' AND c.veterinario_id = :id_veterinario");
...
// Obtener listado de citas realizadas para este veterinario
try {
    ...
    WHERE c.estado = 'realizada' AND c.veterinario_id = :id_veterinario
    ...
    error_log("Error al obtener citas realizadas: " . $e->getMessage());
    $citas = [];
}
include_once '../templates/header.php';
?>
<link rel="stylesheet" href="../app/css/veterinario/gestion_citas_realizadas.css">
                ...
                <?php if (empty($citas)): ?>
                    <tr>
                        <td colspan="6" style="text-align:center; padding:50px 20px; color:#8fa3b3;">
                            ...
                            <p style="margin-top: 10px; font-size: 0.9rem;">No hay citas realizadas registradas.</p>
                        </td>
                    </tr>
                <?php else: ?>
...
<script src="../app/js/veterinario/gestion_citas_realizadas.js"></script>
