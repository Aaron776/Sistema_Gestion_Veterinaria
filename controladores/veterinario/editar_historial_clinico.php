...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_historial_clinico']) && isset($_POST['id_mascota']) && isset($_POST['id_cita']) && isset($_POST['diagnostico']) && isset($_POST['tratamiento']) && isset($_POST['observaciones']) && isset($_POST['peso']) && isset($_POST['sintomas'])) {
    // Validar token CSRF
    ...
    if (empty($errores)) {
        try {
            $sql_historial_clinico = $conexion->prepare("SELECT id FROM historial_clinico WHERE id = :id_historial_clinico");
    ...
    // Si no hay errores, editar historial clinico
    if (empty($errores)) {
        try {
            ...
            $sql = $conexion->prepare("UPDATE historial_clinico SET mascota_id = :id_mascota, veterinario_id = :id_veterinario, cita_id = :id_cita, peso = :peso, sintomas = :sintomas, diagnostico = :diagnostico, tratamiento = :tratamiento, observaciones = :observaciones WHERE id = :id_historial_clinico");
            ...
            if ($recepcionistas) {
                $titulo_not = "Historial Clínico Editado: " . ucfirst($pet_name);
                $mensaje_not = "El veterinario ha editado el historial para " . ucfirst($pet_name) . " (Cita #$id_cita). Ya puede proceder con el flujo de cierre.";
            ...
            $_SESSION['exito'] = "Historial clinico editado exitosamente";
            header("Location: ../../veterinario/historial_clinico_mascota.php?id_mascota=" . urlencode(Crypto::encrypt($id_mascota)));
            exit;
        } catch (PDOException $e) {
            $conexion->rollBack();
            error_log("Error al editar el historial clinico: " . $e->getMessage());
            $errores[] = "Error al editar el historial clinico";
            $_SESSION['errores'] = $errores;
            header("Location: ../../veterinario/editar_historial_clinico.php?id_mascota=" . urlencode(Crypto::encrypt($id_mascota)) . "&id_cita=" . urlencode(Crypto::encrypt($id_cita)) . "&id_historial_clinico=" . urlencode(Crypto::encrypt($id_historial_clinico)));
            exit;
        }
    } else {
        $_SESSION['errores'] = $errores;
    ...
    $_SESSION['errores'] = ["Error al editar el historial clinico"];
